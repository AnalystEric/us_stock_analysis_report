# 美股／台股投資分析 PDF 報告產生器（投行等級深度報告）

輸入**股票代號**或**公司名稱**，自動判別市場並查詢公開資料，
經 Pandas 分析、專業圖表、（選用）LLM 質化分析與 **WeasyPrint (HTML→PDF)** 排版，
產出一份**如同 sell-side 研究報告**的中文（中英夾雜）PDF。

- **美股**：`AAPL`、`NVDA`、`Apple`…（以 [`yfinance`](https://github.com/ranaroussi/yfinance) 為核心）
- **台股**：`2330`、`6488`、`台積電`…（純數字自動判別上市 `.TW` / 上櫃 `.TWO`）
  - 以**台灣官方源為主**：TWSE／TPEX OpenAPI、公開資訊觀測站
  - 台股特有區塊：**月營收 + YoY／MoM**、**三大法人買賣超**、**融資融券餘額**
  - 金額改以 **NT$／億／兆** 呈現；美股維持 `$／T/B/M`
